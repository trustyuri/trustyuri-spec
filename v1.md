...
Version 1 described here is not backward compatible with version 0 (the handling
of plain literals according to RDF 1.1 breaks compatibility). Future versions,
however, are supposed to be 100% backward compatible with this version (by
deprecating modules and defining new ones, if modules cannot be extended in a
...
serialized as an at-sign `@` followed by the language string, a blank space, and
